...
const SchemaContext = /*#__PURE__*/React.createContext({});

const typeToRegex = type => new RegExp(`^${type}:`);

const queriesToPath = queries => queries.map(query => [ROOT, new RegExp(`^${query}`)]); // Turns this
...
const generateInvalidateionPaths = memoizeOne(keystoneTypes => keystoneTypes.reduce((memo, {
  schema: {
    type,
    queries = [],
    relatedFields = []
...
const KeystoneQuery = (_ref) => {
  ...
  return /*#__PURE__*/React.createElement(Query, props, renderProps => /*#__PURE__*/React.createElement(KeystoneApolloQueryCacheBuster, _extends({}, renderProps, {
    children: children
  })));
};
...
class KeystoneMutation extends React.Component {
  render() {
    ...
    return /*#__PURE__*/React.createElement(SchemaContext.Consumer, null, keystoneSchemaInfo => {
      const interceptedUpdateProp = wrapUpdateFunc(keystoneSchemaInfo, invalidateTypes, updateProp);
      return /*#__PURE__*/React.createElement(Mutation, _extends({}, props, {
        ...
        const interceptedMutation = (_ref2) => {
          let {
            ...
            optimisticResponse
              ...
              rest = _objectWithoutProperties(_ref2, ["update", "optimisticResponse"]);
...
class KeystoneProvider extends React.Component {
  render() {
    return /*#__PURE__*/React.createElement(KeystoneQuery, {
      query: META_QUERY
    }, ({
      data,
      loading
...
const withKeystone = Component => {
  const WrappingComponent = props => /*#__PURE__*/React.createElement(KeystoneProvider, null, /*#__PURE__*/React.createElement(Component, props));
...
function injectIsOptimisticFlag(_ref3 = emptyObject) {
  let {
    ...
    defaults = emptyObject
      ...
      rest = _objectWithoutProperties(_ref3, ["resolvers", "defaults"]);

  const injectIsOptimisticFlagToKeystoneTypes = memoizeOne((inputResolvers, keystoneTypes) => (keystoneTypes || []).reduce((memo, {
    schema: {
      type
    }
  ...
  return (_ref4) => {
    let {
      render
        ...
        props = _objectWithoutProperties(_ref4, ["render"]);

    return /*#__PURE__*/React.cloneElement(adoption, props, render);
  };
}));
